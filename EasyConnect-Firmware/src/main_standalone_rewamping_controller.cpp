...
bool apiAuthRejected = false;               // True se l'ultimo invio API ha ricevuto 401/403 (chiave/API non valida).
bool apiCustomerIssue = false;              // True se API cliente non configurata o in errore autenticazione.
bool slaveExcludedByPortal[101] = {false};  // True se seriale escluso dal portale (retired/voided).
float currentDeltaP = 0.0;                  // Valore corrente del Delta P calcolato.
bool currentDeltaPValid = false;            // True se currentDeltaP proviene da due misure valide.
...
static const unsigned long REMOTE_SYNC_NORMAL_MS = 60000UL; // Invio cloud in esercizio normale.
static const unsigned long REMOTE_SYNC_TESTWIZ_MS = 2000UL; // Invio cloud durante test wizard DeltaP.
...
static bool standaloneWifiStartRequested = false;
static bool standaloneWifiServicesActive = false;
static bool standaloneOtaInitialized = false;

void requestStandaloneWifiPortalStart() {
    standaloneWifiStartRequested = true;
}
...
void setup() {
    ...
    config.sogliaManutenzione = memoria.getInt("m_life_h", 9000);
    if (config.sogliaManutenzione < 0) config.sogliaManutenzione = 0;
    ...
    Serial.println("\n--- EASY CONNECT CONTROLLER (Boot Complete) ---");

    // Se la scheda non è configurata, entra in un loop di blocco.
    while (!config.configurata) {
        static bool bootHintPrinted = false;
        if (!bootHintPrinted) {
            Serial.println("[!] CONTROLLER NON CONFIGURATO. Inserire: SETSERIAL...");
            Serial.println("Digitare 'HELP' per la lista comandi.");
            bootHintPrinted = true;
        }
    ...
    if (config.modalitaMaster == 1) {
        ...
        const bool standaloneWifiBoot = memoria.getBool("st_wifi_boot", false);
        if (standaloneWifiBoot) {
            Serial.println("[STANDALONE] WiFi locale abilitato da flag persistente.");
            setupStandaloneWifiPortal();
            setupOTA();
            standaloneWifiServicesActive = true;
            standaloneOtaInitialized = true;
        } else {
            // In standalone senza persistenza WiFi, il reboot deve ripartire "prima installazione":
            // nessuna rete esterna ricordata.
            memoria.remove("ssid");
            memoria.remove("pass");
            memoria.remove("static_ip");
            memoria.remove("ip");
            memoria.remove("sub");
            memoria.remove("gw");
            Serial.println("[STANDALONE] Flag WiFi boot OFF: credenziali WiFi esterne cancellate.");
...
void loop() {
    ...
    if (config.modalitaMaster == 1) {
        ...
        if (standaloneWifiStartRequested) {
            standaloneWifiStartRequested = false;
            if (!standaloneWifiServicesActive) {
                Serial.println("[STANDALONE] Avvio WiFi locale richiesto da pulsante.");
                standalonePlayWifiStartAnimation();
                setupStandaloneWifiPortal();
                if (!standaloneOtaInitialized) {
                    setupOTA();
                    standaloneOtaInitialized = true;
                }
                standaloneWifiServicesActive = true;
            } else {
                Serial.println("[STANDALONE] WiFi locale gia' attivo.");
            }
        }
        ...
        if (standaloneWifiServicesActive) {
            ...
            if (!manualOtaActive && !otaSlaveActive) {
                unsigned long ora = millis();
                if (ora - timerRemoteSync >= REMOTE_SYNC_NORMAL_MS) {
                    sendDataToRemoteServer();
                    timerRemoteSync = ora;
                }
                if ((ora - timerUpdateCheck >= 30000) || (timerUpdateCheck == 0 && WiFi.status() == WL_CONNECTED)) {
        ...
        if (!manualOtaActive) {
            RS485_Controller_Loop(); // Contiene sia il polling che la macchina a stati dell'OTA automatico
            ...
            if (!otaSlaveActive) {
                ...
                // - normale: ogni 60s
                // - durante TESTWIZ (seriale/web): ogni 2s per campionamento piu' fitto sul portale
                const unsigned long remoteSyncIntervalMs =
                    webIsDeltaPTestWizardBusy() ? REMOTE_SYNC_TESTWIZ_MS : REMOTE_SYNC_NORMAL_MS;
                if (ora - timerRemoteSync >= remoteSyncIntervalMs) {
                    sendDataToRemoteServer();
                    timerRemoteSync = ora;
                }
                // Controllo aggiornamenti/comandi periodico (ogni 30s per reattivita da portale)
                if ((ora - timerUpdateCheck >= 30000) || (timerUpdateCheck == 0 && WiFi.status() == WL_CONNECTED)) { 
                    checkForFirmwareUpdates();
                    timerUpdateCheck = ora;
                }
