...
struct HomeTile {
    ...
    uint8_t addresses[DC_MAX_DEVICES];
...
static HomeTile s_home_tiles[DC_MAX_DEVICES];
static SavedRelayState s_saved_relay_states[DC_MAX_DEVICES];
...
static void _refresh_env_labels() {
    ...
    if (!g_dc_model.environment.valid) {
        lv_label_set_text(s_temp_lbl, is_fahrenheit ? "-- F" : "-- C");
        lv_label_set_text(s_hum_lbl, "-- %RH");
        return;
    }

    const float env_temp = g_dc_model.environment.temp_c;
    const float env_hum  = g_dc_model.environment.hum_rh;

    const float temp_value = is_fahrenheit
        ? ((env_temp * 9.0f / 5.0f) + 32.0f)
        : env_temp;

    const int temp_tenths = (int)(temp_value * 10.0f + (temp_value >= 0.0f ? 0.5f : -0.5f));
    const int hum_tenths = (int)(env_hum * 10.0f + 0.5f);
...
static bool _find_device_by_address(uint8_t address, DcDeviceSnapshot& out) {
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        ...
        if (dev.address == address) { out = dev; return true; }
    }
    return false;
}

static bool _device_has_comm_issue(const DcDeviceSnapshot& dev) {
    return dev.valid && !dev.online;
}

static bool _relay_has_safety_issue(const DcDeviceSnapshot& dev) {
    if (dev.type != DC_DEV_RELAY) return false;
    if (dev.relay_mode != DC_RELAY_UVC && dev.relay_mode != DC_RELAY_ELECTRO) return false;
    if (!dev.online) return false;
    return dev.safety_fault;
}

static bool _relay_has_feedback_fault(const DcDeviceSnapshot& dev) {
    if (dev.type != DC_DEV_RELAY) return false;
    if (dev.relay_mode != DC_RELAY_UVC && dev.relay_mode != DC_RELAY_ELECTRO) return false;
    if (!dev.online) return false;
    return dev.feedback_fault;
}

static bool _air010_has_feedback_fault(const DcDeviceSnapshot& dev) {
    if (dev.type != DC_DEV_MOTOR_010) return false;
    if (!dev.online || !dev.motor_enabled) return false;
    return dev.feedback_fault;
}

static bool _device_has_warning(const DcDeviceSnapshot& dev) {
...
static bool _is_pressure_sensor(const DcDeviceSnapshot& dev) {
    return dev.valid && dev.type == DC_DEV_SENSOR;
}
...
static int _collect_pressure_groups(PressureGroupSummary* groups, int max_groups) {
    ...
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        ...
        grp.online_count++;
        grp.pressure_sum += dev.pressure_pa;
        grp.pressure_count++;
        grp.temp_sum += dev.temp_c;
        grp.temp_count++;
    }
...
static void _upsert_saved_relay_state(uint8_t address, uint8_t relay_mode, bool desired_on) {
    ...
    if (idx < 0) {
        if (s_saved_relay_count >= DC_MAX_DEVICES) return;
...
static void _sync_saved_relay_states_from_runtime() {
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        const DcDeviceSnapshot& dev = g_dc_model.network.devices[i];
        if (!dev.valid) continue;
        if (dev.type != DC_DEV_RELAY) continue;
        _upsert_saved_relay_state(dev.address, (uint8_t)dev.relay_mode, dev.relay_on);
    }
}
...
static void _apply_saved_relay_states(bool include_protected) {
    for (int i = 0; i < s_saved_relay_count; i++) {
        ...
        const bool desired = should_force_off ? false : entry.desired_on;
        dc_cmd_relay_set(entry.address, desired);
    }
}
...
static bool _has_protected_systems_in_plant(bool* has_uvc, bool* has_electro) {
    ...
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        ...
        if (dev.relay_mode == DC_RELAY_UVC)     uvc = true;
        if (dev.relay_mode == DC_RELAY_ELECTRO) electro = true;
    }
...
static bool _has_any_active_relay(void) {
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        const DcDeviceSnapshot& dev = g_dc_model.network.devices[i];
        if (!dev.valid || !dev.online) continue;
        if (dev.type != DC_DEV_RELAY) continue;
...
static void _shutdown_all_plant_relays() {
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        ...
        dc_cmd_relay_set(dev.address, false);
    }
...
static void _refresh_bypass_glow() {
    ...
    if (g_dc_model.system_bypass_active) lv_obj_clear_flag(s_bypass_glow, LV_OBJ_FLAG_HIDDEN);
...
static void _safety_popup_bypass_cb(lv_event_t* e) {
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) return;
    g_dc_model.system_bypass_active = true;
...
static const char* _tile_warning_title(const DcDeviceSnapshot& dev) {
    if (dev.type == DC_DEV_RELAY) {
        return (dev.relay_mode == DC_RELAY_UVC)     ? "UVC" :
               (dev.relay_mode == DC_RELAY_ELECTRO) ? "Elettrostatico" :
               "Relay";
    }
    if (dev.type == DC_DEV_MOTOR_010) {
...
static void _format_notification_body(const DcDeviceSnapshot& dev, const char* issue,
                                      char* out, size_t out_size) {
    if (!out || out_size == 0) return;
...
static void _refresh_wifi_indicator() {
    ...
    const int rssi = g_dc_model.wifi.rssi;
    lv_color_t clr;
    if      (rssi >= -55) clr = lv_color_hex(0xE84820);
    else if (rssi >= -67) clr = lv_color_hex(0xEE7848);
    else if (rssi >= -75) clr = lv_color_hex(0xE8A888);
    else                  clr = lv_color_hex(0x9EB0C4);
...
void ui_dc_home_set_environment(float temp_c, float hum_rh, bool valid) {
    (void)temp_c; (void)hum_rh; (void)valid;
    _refresh_env_labels();
}
...
static void _open_settings_cb(lv_event_t* e) {
    if (lv_event_get_code(e) != LV_EVENT_CLICKED) return;
    lv_obj_t* settings = ui_settings_shared_create();
...
static bool _device_to_tile_key(const DcDeviceSnapshot& dev,
                                HomeTileKind& kind,
                                uint8_t& group,
                                bool& is_relay,
                                bool& is_on) {
    ...
    if (dev.type == DC_DEV_RELAY) {
        if (dev.relay_mode == DC_RELAY_GAS)     return false;
        if (dev.relay_mode == DC_RELAY_LIGHT)   kind = HomeTileKind::RELAY_LIGHT;
        else if (dev.relay_mode == DC_RELAY_UVC)     kind = HomeTileKind::RELAY_UVC;
        else if (dev.relay_mode == DC_RELAY_ELECTRO) kind = HomeTileKind::RELAY_ELECTRO;
        else if (dev.relay_mode == DC_RELAY_COMMAND) kind = HomeTileKind::RELAY_COMMAND;
        else return false;
        group = dev.group;
        is_relay = true;
        is_on = dev.relay_on;
...
static int _send_air_speed_command(HomeTile& tile, int output_pct) {
    ...
    for (uint8_t i = 0; i < tile.addresses_count; i++) {
        if (dc_cmd_motor_speed(tile.addresses[i], (uint8_t)output_pct)) ok_count++;
    }
    if (ok_count > 0) {
...
static bool _refresh_home_tile_state_from_network(HomeTile& tile) {
    ...
    for (uint8_t i = 0; i < tile.addresses_count; i++) {
        DcDeviceSnapshot dev;
        if (!_find_device_by_address(tile.addresses[i], dev)) continue;
        refreshed_count++;
...
static void _sync_home_tiles_from_network_state() {
    for (int i = 0; i < s_home_tile_count; i++) {
        ...
        for (uint8_t j = 0; j < tile.addresses_count; j++) {
            DcDeviceSnapshot dev;
            if (!_find_device_by_address(tile.addresses[j], dev)) continue;

            const bool is_on = tile.is_relay ? dev.relay_on : dev.motor_enabled;
            if (is_on) tile.on_count++;
            if (_device_has_warning(dev)) tile.has_warning = true;
            if (_is_air_010_tile(tile)) {
                tile.speed_sum += _sanitize_speed_pct((int)dev.speed_pct);
                tile.speed_count++;
                if (dev.group == 1) {
...
static void _sync_notifications_from_network_state() {
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        ...
        if (dev.type != DC_DEV_RELAY) continue;
        if (dev.relay_mode != DC_RELAY_UVC && dev.relay_mode != DC_RELAY_ELECTRO) continue;
...
static void _handle_system_safety_state() {
    ...
    if (!safety_active) {
        if (g_dc_model.system_safety_trip) {
            _apply_saved_relay_states(true);
        }
        g_dc_model.system_safety_trip = false;
        g_dc_model.system_bypass_active = false;
        _dismiss_safety_popup();
        _refresh_bypass_glow();
    ...
    if (g_dc_model.system_bypass_active) {
        lv_snprintf(body, sizeof(body),
    ...
    if (!g_dc_model.system_safety_trip) {
        ...
        g_dc_model.system_safety_trip = true;
        _shutdown_all_plant_relays();
        if (had_active_relays && !g_dc_model.system_bypass_active) {
...
static void _home_tile_click_cb(lv_event_t* e) {
    ...
    if (_is_air_010_tile(tile)) {
        ...
        int ok_count = 0;
        for (uint8_t i = 0; i < tile.addresses_count; i++) {
            if (dc_cmd_motor_enable(tile.addresses[i], target_on)) ok_count++;
        }
    ...
    if (target_on && safety_active) {
        _shutdown_all_plant_relays();
        if (!g_dc_model.system_bypass_active || _is_protected_tile_kind(tile.kind)) {
            _show_safety_popup(has_uvc, has_electro);
            _refresh_notification_bell();
            return;
        }
    ...
    if (target_on && g_dc_model.system_bypass_active && _is_protected_tile_kind(tile.kind)) {
        _show_safety_popup(has_uvc, has_electro);
        _refresh_notification_bell();
        return;
    }
    ...
    int ok_count = 0;
    for (uint8_t i = 0; i < tile.addresses_count; i++) {
        if (dc_cmd_relay_set(tile.addresses[i], target_on)) ok_count++;
    }
...
static void _build_home_tiles_from_network() {
    ...
    for (int i = 0; i < g_dc_model.network.device_count; i++) {
        ...
        if (idx < 0) {
            if (s_home_tile_count >= DC_MAX_DEVICES) continue;
        ...
        if (tile.addresses_count < DC_MAX_DEVICES) {
            tile.addresses[tile.addresses_count++] = dev.address;
        }
    }

    if (_has_intake_and_extraction_tiles() && s_home_tile_count < DC_MAX_DEVICES) {
        ...
        for (int i = 0; i < combined_idx; i++) {
            ...
            for (uint8_t j = 0; j < tile.addresses_count; j++) {
                if (combined.addresses_count < DC_MAX_DEVICES) {
...
lv_obj_t* ui_tc_home_create(void) {
